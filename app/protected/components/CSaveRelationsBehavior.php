...
class CSaveRelationsBehavior extends CActiveRecordBehavior {
				...
				if(!is_null($data)) {
					foreach($data as $key=>$value) {
						...
						if(!$model->isNewRecord) {
							$relModel = new $relationClassName;
							if(is_array($value)) {
								$criteria = array($relationForeignKey=>$model->primaryKey);
								...
								if(is_array($relationPrimaryKeys)) {
									foreach($relationPrimaryKeys as $relationPrimaryKey){
										if(!in_array($relationPrimaryKey,$fks)) $criteria[$relationPrimaryKey] = $value[$relationPrimaryKey];
									}
								}
								else{
									$criteria = array($relationPrimaryKeys=>$model->primaryKey);
								}
							}
							else {
								$criteria = array($relModel->tableSchema->primaryKey=>$value);
							}
	...
	public function addSaveRelation($relation,$message=null){
		...
		if(!array_key_exists($relation,$model->relations())) 
			throw new CDbException('CSaveRelated could not find the "'.$relation.'" relation in the model.');
		if(!array_key_exists($relation,$this->relations)) {
			Yii::trace("setRelatedRecords adding {$relation} relation to save",'application.components.CSaveRelatedBehavior');
	...
	public function removeSaveRelation($relation){
		...
		if(!array_key_exists($relation,$model->relations())) 
			throw new CDbException('CSaveRelated could not find the "'.$relation.'" relation in the model.');
		if(array_key_exists($relation,$this->relations)) {
			Yii::trace("setRelatedRecords removing {$relation} relation to save",'application.components.CSaveRelatedBehavior');
	...
	public function setSaveRelationMessage($relation,$message) {
		...
		if(!array_key_exists($relation,$model->relations())) 
			throw new CDbException('CSaveRelated could not find the "'.$relation.'" relation in the model.');
		if(array_key_exists($relation,$this->relations)) {
			Yii::trace("setRelatedRecords setting {$relation} relations message",'application.components.CSaveRelatedBehavior');
	...
	public function beforeSave($event) {
		...
		foreach($this->relations as $relation=>$params) {
			if(isset($params['save']) && $params['save']==true) {
				$valid = $valid && isset($this->relations[$relation]['valid']) && $this->relations[$relation]['valid'];
			}
		}
