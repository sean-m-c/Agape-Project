...
f.tools=f.tools||{version:"1.2.3"};f.tools.tooltip={conf:{effect:"toggle",fadeOutSpeed:"fast",predelay:0,delay:30,opacity:1,tip:0,position:["top","center"],offset:[0,0],relative:false,cancelDefault:true,events:{def:"mouseenter,mouseleave",input:"focus,blur",widget:"focus mouseenter,blur mouseleave",tooltip:"mouseenter,mouseleave"},layout:"<div/>",tipClass:"tooltip"},addEffect:function(a,b,c){n[a]=[b,c]}};var n={toggle:[function(a){var b=this.getConf(),c=this.getTip();b=b.opacity;b<1&&c.css({opacity:b});c.show();a.call()},function(a){this.getTip().hide();a.call()}],fade:[function(a){var b=this.getConf();this.getTip().fadeTo(b.fadeInSpeed,b.opacity,a)},function(a){this.getTip().fadeOut(this.getConf().fadeOutSpeed,a)}]};f.fn.tooltip=function(a){var b=this.data("tooltip");if(b)return b;a=f.extend(true,{},f.tools.tooltip.conf,a);if(typeof a.position=="string")a.position=a.position.split(/,?\s/);this.each(function(){b=new t(f(this),a);f(this).data("tooltip",b)});return a.api?b:this}})(jQuery);;(function($){$.fn.yiiGridView=function(options){return this.each(function(){var settings=$.extend({},$.fn.yiiGridView.defaults,options||{});var $this=$(this);var id=$this.attr('id');if(settings.updateSelector==undefined){settings.updateSelector='#'+id+' .'+settings.pagerClass+' a, #'+id+' .'+settings.tableClass+' thead th a';}
$.fn.yiiGridView.settings[id]=settings;if(settings.ajaxUpdate.length>0){$(settings.updateSelector).live('click',function(){$.fn.yiiGridView.update(id,{url:$(this).attr('href')});return false;});}
var inputSelector='#'+id+' .'+settings.filterClass+' input, '+'#'+id+' .'+settings.filterClass+' select';$(inputSelector).live($.browser.msie?'click keyup':'change',function(){var data=$.param($(inputSelector));$.fn.yiiGridView.update(id,{data:data});});if(settings.selectableRows>0){$('#'+id+' .'+settings.tableClass+' > tbody > tr').live('click',function(){if(settings.selectableRows==1)
$(this).siblings().removeClass('selected');$(this).toggleClass('selected');if(settings.selectionChanged!=undefined)
settings.selectionChanged(id);});}});};$.fn.yiiGridView.defaults={ajaxUpdate:[],ajaxVar:'ajax',pagerClass:'pager',loadingClass:'loading',filterClass:'filters',tableClass:'items',selectableRows:1};$.fn.yiiGridView.settings={};$.fn.yiiGridView.getKey=function(id,row){return $('#'+id+' > div.keys > span:eq('+row+')').text();};$.fn.yiiGridView.getUrl=function(id){return $('#'+id+' > div.keys').attr('title');};$.fn.yiiGridView.getRow=function(id,row){var settings=$.fn.yiiGridView.settings[id];return $('#'+id+' .'+settings.tableClass+' > tbody > tr:eq('+row+') > td');};$.fn.yiiGridView.getColumn=function(id,column){var settings=$.fn.yiiGridView.settings[id];return $('#'+id+' .'+settings.tableClass+' > tbody > tr > td:nth-child('+(column+1)+')');};$.fn.yiiGridView.update=function(id,options){var settings=$.fn.yiiGridView.settings[id];$('#'+id).addClass(settings.loadingClass);options=$.extend({type:'GET',url:$.fn.yiiGridView.getUrl(id),success:function(data,status){$.each(settings.ajaxUpdate,function(i,v){var id='#'+v,$d=$(data)
$filtered=$d.filter(id);$(id).replaceWith($filtered.size()?$filtered:$d.find(id));});if(settings.afterAjaxUpdate!=undefined)
...
if(settings.ajaxUpdate!==false){options.url=$.param.querystring(options.url,settings.ajaxVar+'='+id);if(settings.beforeAjaxUpdate!=undefined)
settings.beforeAjaxUpdate(id,options);$.ajax(options);}
else{if(options.type=='GET'){window.location.href=options.url;}
else{var $form=$('<form action="'+options.url+'" method="post"></form').appendTo('body');if(options.data==undefined){options.data={};}
if(options.data['returnUrl']==undefined){options.data['returnUrl']=window.location.href;}
$.each(options.data,function(name,value){$form.append($('<input type="hidden" name="t" value="" />').attr('name',name).val(value));});$form.submit();}}};$.fn.yiiGridView.getSelection=function(id){var settings=$.fn.yiiGridView.settings[id];var keys=$('#'+id+' > div.keys > span');var selection=[];$('#'+id+' .'+settings.tableClass+' > tbody > tr').each(function(i){if($(this).hasClass('selected'))
selection.push(keys.eq(i).text());});return selection;};})(jQuery);jQuery(document).ready(function(){if($(".taskNavLink").length){jQuery('.taskNavLink').click(function(){jQuery('#taskPanel').fadeOut('fast').html('<div class="loading"></div>').fadeIn('fast');jQuery('.taskNavLink').removeClass('active');jQuery(this).addClass('active');});jQuery('.taskNavLink').tooltip({effect:'fade',position:'center right',opacity:0.85,delay:0,predelay:200});}
if($(".showTooltip").length){jQuery('.showTooltip').tooltip({effect:'fade',position:'center right',opacity:0.85,delay:0,predelay:500,offset:[0,10]});}
var jInput=$(":input");jInput.change(function(objEvent){$("#saved").val("false");});if($("#saveTab").length){$("#saveTab").live('click',function(){$(this).removeClass("i_checkmark").toggleClass('ajaxLoaderSmall');});}
if($("#projectFormTabs").length){$("#projectFormTabs").tabs({select:function(event,ui){if($("#saved").val()!="true"){if(!confirm("You may have unsaved information on this tab. Press \"Ok\" to continue to the next tab, or press \"Cancel\" to go back and save your information.")){return false;}}}});}
if($(".IGSTClick").length){jQuery(".IGSTClick").live('click',function(){var container=$("#ajaxGuiContainer");var loading=$("#loading");container.fadeOut('fast');loading.addClass('loading').fadeIn('fast');IGSTClick(jQuery(this).attr("href"),container,loading);return false;});}});function IGSTClick(url,container,loading){var urlParams=getUrlVars(url);$.ajax({url:url.substring(0,url.indexOf('&')),global:false,type:'GET',data:({id:urlParams['parentID'],ajaxPanel:urlParams['ajaxPanel'],action:urlParams['action']}),dataType:'html',error:function(a,b,c){alert('Error retrieving panel., XMLHttpRequest: '+a+'; textStatus: '+b+' errorThrown: '+c);},success:function(data){loading.fadeOut('fast').removeClass('loading');$('#issue-grid').remove();container.fadeOut('fast').html(data).fadeIn('fast');}});}
function getUrlVars(url){var map={};var parts=url.replace(/[?&]+([^=&]+)=([^&]*)/gi,function(m,key,value){map[key]=value;});return map;}
function gridButtonClick(url,gridId){var id=url.substring(url.indexOf('id=')+3);$.fn.yiiGridView.update(gridId,{type:'POST',url:url.substring(0,url.indexOf('&')),data:{'ajaxData':{'id':id}},success:function(data,status){$.fn.yiiGridView.update(gridId);if(data!=''){alert(data);}}});}
function loadPanel(panelID){var urlHost=document.location.href.substr(0,document.location.href.indexOf('?'));var controller=document.location.href.substr(document.location.href.indexOf('r=')+2,document.location.href.lastIndexOf('/')-document.location.href.indexOf('r=')-2);jQuery.ajax({'success':function(data){jQuery('#taskNav #'+panelID).addClass('active');$('#taskPanel').html(data).fadeIn('fast');location.hash=panelID+'Panel';},'url':urlHost+'?r='+controller+'/renderTaskPanel&panel='+panelID});}